...
export default function BienesPage() {
  ...
  const loadIncoming = async () => {
    ...
    try {
      const response = await APIClient.get<any>('/desplazamientos');
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <CardTitle className="text-lg">
                  {user?.id_tipo_cargo === 1 ? 'Todos los bienes' : 'Mis bienes'}
                </CardTitle>
                      ...
                      <thead className="bg-background border-b border-border/40 sticky top-0">
                        <tr>
                          ...
                          {user?.id_tipo_cargo === 1 && (
                            <th className="px-4 py-3 text-left font-semibold text-foreground">Usuario Asignado</th>
                          )}
                ...
                return (
                  <div key={request.id_desplazamiento} className={`rounded-xl border p-4 shadow-sm ${isAdminManage ? 'border-amber-500/30 bg-amber-500/5' : 'border-border/40 bg-background'}`}>
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <p className="text-sm font-medium">De: {request.origen_nombre}</p>
                          <ArrowRight size={14} className="text-muted-foreground" />
                          <p className="text-sm font-medium">Para: {request.destino_nombre}</p>
                        </div>
                        <p className="text-sm italic text-muted-foreground">"{request.razon || 'Sin motivo especificado'}"</p>
                        <p className="text-sm text-muted-foreground">Bienes: {request.bienes_nombres || 'No disponible'}</p>
                        <p className="text-sm text-muted-foreground">Fecha: {new Date(request.fecha_inicio).toLocaleDateString()}</p>
                        {isAdminManage && <span className="text-[10px] font-bold uppercase text-amber-600 bg-amber-600/10 px-1.5 py-0.5 rounded">Gestión Admin</span>}
                      </div>
                      <div className="flex gap-2">
                        <Button variant="outline" onClick={() => handleIncomingAction(request.id_desplazamiento, false)}>
                          <XCircle size={16} /> {isAdminManage ? 'Cancelar' : 'Rechazar'}
                        </Button>
                        <Button onClick={() => handleIncomingAction(request.id_desplazamiento, true)}>
                          <CheckCircle2 size={16} /> {isAdminManage ? 'Confirmar' : 'Aceptar'}
                        </Button>
                      </div>
                    </div>
                  </div>
                ...
                <div key={request.id_desplazamiento} className="rounded-xl border border-border/40 bg-background p-4 shadow-sm">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Bienes: {request.bienes_nombres || 'No disponible'}</p>
                      <p className="text-sm text-muted-foreground">Estado: {request.id_estado === 3 ? 'En Proceso' : request.id_estado === 4 ? 'Completado' : request.id_estado === 2 ? 'Cancelado' : 'Rechazado'}</p>
                    </div>
                    {request.id_estado === 3 && (
